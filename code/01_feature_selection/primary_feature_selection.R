...
#   3) 因此 38 个原始候选 -> 36 个先验候选
#   4) 在固定主分析训练集内再做技术性零方差检查；零方差变量不进入 LASSO/Boruta
#      （按当前数据，Cholinesterase inhibitors 在训练集为全0，因此预计 35 个变量实际进入算法）
#   5) 使用主分析已经固定的原始 Train/Test 身份，
#      不再用 caret::createDataPartition 重新随机拆分
#   6) 单独输出 Age 的 LASSO 系数和 Boruta Z-score/decision
#   7) 增加20次重复稳定性分析：
...
# Set AIS_ICU_DATA_FILE to the authorized local analysis-ready CSV, or place the
# expected file under ./data/BSAfree_feature_selection_input_exactsplit.csv.
...
INPUT_FILE <- Sys.getenv(
  "AIS_ICU_DATA_FILE",
  unset = file.path(REPO_ROOT, "data", "BSAfree_feature_selection_input_exactsplit.csv")
)
SAVE_PATH <- Sys.getenv(
  "AIS_ICU_FEATURE_SELECTION_OUTPUT",
  unset = file.path(REPO_ROOT, "outputs", "feature_selection")
)
...
A_PRIORI_EXCLUDED <- c("BSA", "Immunosuppressants")
...
if (length(PREDICTORS_AFTER_EXCLUSION) != 36) {
  stop("删除BSA和Immunosuppressants后不是36个候选变量，请检查。")
}
...
write.csv(
  data.frame(
    Variable = c("BSA", "Immunosuppressants"),
    Exclusion_stage = c("A priori before feature selection", "A priori before feature selection"),
    Reason = c(
...
method_note <- c(
  ...
  paste0("- Candidate count after the two a priori exclusions: ", length(PREDICTORS_AFTER_EXCLUSION), " (expected 36)."),
...
cat("  01_主分析36候选变量_零方差与稀有度核查.csv\n")
