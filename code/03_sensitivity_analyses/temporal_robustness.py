# -*- coding: utf-8 -*-
"""
Temporal robustness analysis for the current 11-predictor primary models.

Development period: 2020-2023
Later temporal cohort: 2024-2025

The statistical/modeling logic is preserved from the revision analysis. Patient-level
inputs are intentionally not included in the public repository. Set AIS_ICU_DATA_DIR
to a local restricted-data directory, or set AIS_ICU_TEMPORAL_INPUT_FILE to the
specific archived analysis-ready CSV. Outputs are written under AIS_ICU_OUTPUT_DIR
...
TEMPORAL_INPUT_ENV = os.environ.get("AIS_ICU_TEMPORAL_INPUT_FILE", "").strip()
output_path = str(OUTPUT_ROOT / "temporal_robustness")
...
FEATURES_USED = [
    ...
    'Surgery',
    'Diuretics',
    'CO2',                          # manuscript display: TCO2
]

# ★★★ 时间分层划分配置 ★★★
TIME_VAR    = 'year'          # 时间变量名
TRAIN_YEARS = [2020, 2021, 2022, 2023]   # 训练集年份
TEST_YEARS  = [2024, 2025]               # 测试集年份


def locate_temporal_input():
    required = {OUTCOME, TIME_VAR, *FEATURES_USED}

    if TEMPORAL_INPUT_ENV:
        p = Path(TEMPORAL_INPUT_ENV).expanduser().resolve()
        if not p.exists():
            raise FileNotFoundError(f"AIS_ICU_TEMPORAL_INPUT_FILE does not exist: {p}")
        return p

    preferred = [
        DATA_ROOT / "temporal_analysis_input.csv",
        DATA_ROOT / "英文列名准备变量筛选.csv",
    ]
    candidates = [p for p in preferred if p.exists()]
    candidates.extend(
        p for p in DATA_ROOT.rglob("*.csv")
        if p not in candidates and "outputs" not in {x.name for x in p.parents}
    )

    valid = []
    for p in candidates:
        try:
            d = pd.read_csv(p, encoding="utf-8-sig", nrows=20)
            d.columns = d.columns.astype(str).str.replace("\ufeff", "", regex=False).str.strip()
            if not required.issubset(d.columns):
                continue
            full = pd.read_csv(p, encoding="utf-8-sig", usecols=list(required))
            years = set(pd.to_numeric(full[TIME_VAR], errors="coerce").dropna().astype(int).unique())
            if set(TRAIN_YEARS + TEST_YEARS).issubset(years):
                valid.append(p)
        except Exception:
            continue
    ...
    if not valid:
        raise FileNotFoundError(
            "Could not locate a temporal analysis input under "
            f"AIS_ICU_DATA_DIR={DATA_ROOT}. Set AIS_ICU_TEMPORAL_INPUT_FILE explicitly."
        )
    return sorted(valid, key=lambda x: x.stat().st_mtime, reverse=True)[0]


data_file = str(locate_temporal_input())
...
def choose_youden_threshold_from_probabilities(y_true, y_proba):
    """从真正cross-fitted训练概率中选择Youden最大阈值。"""
...
def strict_cross_fitted_train_probabilities(train_raw_df, y_train_series,
                                             ...
                                             seed=42):
    """
    仅用于训练期分类阈值估计的严格outer cross-fitting：
    1) 每个outer-training fold重新拟合连续变量MinMaxScaler；
    2) 超参数固定为完整development cohort中BayesSearchCV得到的最优值，
       不在outer fold内重新调参；
    3) outer-training内部进行Platt calibration；
    4) 每名development患者只获得一次来自未见过该患者的outer-validation概率。

    注意：这是阈值估计程序，不是完整nested validation。
    """
    y_arr = np.asarray(y_train_series).astype(int)
    raw_df = train_raw_df.reset_index(drop=True).copy()
    outer = StratifiedKFold(n_splits=outer_cv, shuffle=True, random_state=seed)
    oof = np.full(len(raw_df), np.nan, dtype=float)
    fold_rows = []

    for fold, (tr_idx, va_idx) in enumerate(outer.split(raw_df, y_arr), start=1):
        tr_df = raw_df.iloc[tr_idx].copy()
...
print("   [ 11 Features · Temporal Split · Platt Calibration · Strict Cross-fit Threshold ]")
print("="*70)
...
missing = [c for c in FEATURES_USED + [OUTCOME, TIME_VAR] if c not in data.columns]
if missing:
    print(f"\n❌ 缺失列: {missing}"); sys.exit(1)

# 选择最终建模数据（保留时间变量 year 用于时间分层划分）
data = data[[OUTCOME, TIME_VAR] + FEATURES_USED].copy()

# —— 时间变量清洗：仅保留 2020-2025 —— #
data[TIME_VAR] = pd.to_numeric(data[TIME_VAR], errors='coerce')
_n0 = len(data)
data = data[data[TIME_VAR].isin(TRAIN_YEARS + TEST_YEARS)].reset_index(drop=True)
if len(data) < _n0:
    print(f"\n⚠️ 已剔除 year 缺失或不在 {TRAIN_YEARS+TEST_YEARS} 范围内的记录: {_n0-len(data)} 例")
print(f"\n📅 年度分布:")
for _yr, _cnt in data[TIME_VAR].value_counts().sort_index().items():
    print(f"   {int(_yr)}: n={_cnt}")
...
save_csv(data, '建模数据集_11features_含year.csv')
...
# 缺失值核查
# 本时间敏感性分析使用此前已生成的 archived analysis-ready 数据。
# 为避免在时间分析中偷偷引入一套新的全队列中位数/众数填补规则，
# 11个固定预测变量如仍有缺失则直接停止并要求核查输入文件。
missing_info = X_raw.isna().sum()
if missing_info.sum() > 0:
    miss = missing_info[missing_info > 0]
    raise ValueError(
        "时间分层输入的11个固定预测变量仍存在缺失值。"
        "本脚本不会重新进行全队列填补，以避免时间泄漏。\n" + miss.to_string()
    )
else:
    print(f"\n✅ 11个固定预测变量均无缺失值（archived analysis-ready input）")

# 拆分（时间分层：2020-2023 训练 / 2024-2025 测试）+ 标准化
year_series = data[TIME_VAR]
train_mask  = year_series.isin(TRAIN_YEARS).values
test_mask   = year_series.isin(TEST_YEARS).values
...
if len(y_train) == 0 or len(y_test) == 0:
    print(f"\n❌ 时间分层失败：训练集 n={len(y_train)}, 测试集 n={len(y_test)}"); sys.exit(1)

print(f"\n📊 时间分层划分 (变量: {TIME_VAR})")
print(f"   训练集 {TRAIN_YEARS[0]}-{TRAIN_YEARS[-1]}: n={len(y_train)}, "
      f"事件={int(y_train.sum())} ({y_train.mean()*100:.1f}%)")
print(f"   测试集 {TEST_YEARS[0]}-{TEST_YEARS[-1]}: n={len(y_test)}, "
      f"事件={int(y_test.sum())} ({y_test.mean()*100:.1f}%)")

EXPECTED_TEMPORAL = {
    'train_n': 2230, 'train_events': 866,
    'test_n': 1138, 'test_events': 491,
}
_actual = (len(y_train), int(y_train.sum()), len(y_test), int(y_test.sum()))
_expected = (EXPECTED_TEMPORAL['train_n'], EXPECTED_TEMPORAL['train_events'],
             EXPECTED_TEMPORAL['test_n'], EXPECTED_TEMPORAL['test_events'])
if _actual != _expected:
    raise RuntimeError(
        f"时间分层人数/事件数与既往审计不一致: actual={_actual}, expected={_expected}"
    )
print("   ✅ 时间分层人数与事件数核对通过")

# 保存一份方法学说明，避免把该分析误称为完全独立的temporal validation。
with open(os.path.join(output_path, 'README_时间分层11变量_方法学说明.txt'), 'w', encoding='utf-8') as _f:
    _f.write(
        "Temporal sensitivity analysis with the fixed 11-predictor primary set.\n"
        "Development cohort: 2020-2023 (n=2230; events=866).\n"
        "Later temporal cohort: 2024-2025 (n=1138; events=491).\n"
        "The input is the archived analysis-ready dataset created under the earlier preprocessing workflow.\n"
        "Therefore this analysis evaluates robustness to calendar-time partitioning, but is not an independent\n"
        "validation of the complete raw-data preprocessing-to-model-development pipeline.\n"
        "All model tuning, Platt calibration, and operating-threshold estimation are confined to 2020-2023.\n"
        "Thresholds use strict outer-10-fold cross-fitted calibrated development-cohort probabilities with\n"
        "hyperparameters fixed from the full 2020-2023 Bayesian search.\n"
    )

# 划分概况留档
pd.DataFrame({
    'Set':   ['Train', 'Test'],
    'Years': [f"{TRAIN_YEARS[0]}-{TRAIN_YEARS[-1]}", f"{TEST_YEARS[0]}-{TEST_YEARS[-1]}"],
    'N':     [len(y_train), len(y_test)],
    'Events':[int(y_train.sum()), int(y_test.sum())],
    'Event_rate(%)':[round(y_train.mean()*100,2), round(y_test.mean()*100,2)],
}).to_csv(os.path.join(output_path,'时间分层划分概况.csv'),
          index=False, encoding='utf-8-sig')

data.groupby(TIME_VAR).agg(N=(OUTCOME,'size'),
                           Events=(OUTCOME,'sum')).assign(
    **{'Event_rate(%)': lambda d: (d['Events']/d['N']*100).round(2)}
).reset_index().to_csv(os.path.join(output_path,'年度结局发生率.csv'),
                       index=False, encoding='utf-8-sig')
...
print(f"     3) Youden threshold from strict outer-10-fold cross-fitted calibrated development probabilities")
...
strict_oof_probas={}      # 仅用于development cohort阈值估计
...
for name, config in models_config.items():
    ...
    try:
        ...
        # ─── 3. 严格outer 10-fold cross-fitting生成development期OOF概率，并据此锁定Youden阈值 ───
        print("  ⚙️  生成strict cross-fitted development概率用于阈值估计 ...")
        p_oof, fold_df = strict_cross_fitted_train_probabilities(
            X_train_raw, y_train,
            feature_names=FEATURES_USED,
            continuous_names=exist_cont,
            base_estimator=mdl_raw,
            outer_cv=10,
...
# ─── strict cross-fitted development-cohort probabilities and threshold audit ───
oof_prob_df = pd.DataFrame({
    'Source_row_1based': (np.where(train_mask)[0] + 1),
    'Year': year_series.loc[train_mask].astype(int).values,
...
# ─── 时间分层11变量核心结果：供正文/补充表直接核对 ───
core_rows=[]
for n in model_order:
    bt = boot_test[n]
    core_rows.append({
        'Model': MODEL_FULL_NAME[n],
        'Development_CV_AUC': round(float(cv_aucs[n]), 6),
        'Later_temporal_AUC': round(float(test_results[n]['AUC']), 6),
        'Later_temporal_AUC_95CI_low': round(float(bt['AUC']['ci_lower']), 6),
        'Later_temporal_AUC_95CI_high': round(float(bt['AUC']['ci_upper']), 6),
        'Later_temporal_AP': round(float(test_results[n]['AP']), 6),
        'Later_temporal_Brier': round(float(test_results[n]['Brier Score']), 6),
        'Calibration_intercept': round(float(calib_metrics[n]['test_cal']['intercept']), 6),
        'Calibration_slope': round(float(calib_metrics[n]['test_cal']['slope']), 6),
        'Strict_OOF_threshold': round(float(optimal_thresholds[n]), 6),
        'Development_n': int(len(y_train)),
        'Development_events': int(y_train.sum()),
        'Later_temporal_n': int(len(y_test)),
        'Later_temporal_events': int(y_test.sum()),
    })
save_csv(pd.DataFrame(core_rows), '时间分层11变量_核心结果汇总.csv')
...
def plot_roc_with_auc_bar(y_true, probas_dict, title, save_name,
    ...
    # 截掉Blues最接近白色的底端，避免最低AUC色块与白底融在一起。
    AUC_CMAP_FLOOR = 0.28
    ...
    for rank, rd in enumerate(roc_sorted):
        ...
        ax2.add_patch(plt.Rectangle((0.50, y_bot), 0.50, bar_h,
                                    facecolor=auc_color,
                                    edgecolor='#B7C4CF', linewidth=1.2))
        # AUC 数字：深底白字，浅底黑字
