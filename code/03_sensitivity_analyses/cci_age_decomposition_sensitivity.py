...
output_path = os.path.join(data_path, "CCI分解敏感性分析_12变量_八模型")
...
FEATURES_USED = [
    ...
    'LYMPH_abs',                    # LYM
    'BUN',
    'CCI_unadjusted',               # age-adjusted CCI decomposed: disease-only Charlson score
    'Age',                          # chronological age entered separately
    'FIB',
    'Surgery',
    'Diuretics',
...
print("   [ 12 Features · CCI Decomposition Sensitivity · Fixed Original Split · Platt Calibration · Strict Cross-fit Threshold ]")
print("="*70)
...
# ============================================================
# Reviewer-requested CCI decomposition sensitivity analysis
# Current database variable named CCI is age-adjusted:
#   age-adjusted CCI = disease-only Charlson score + age score
# Age score: <50=0, 50-59=1, 60-69=2, 70-79=3, >=80=4
# The three previously imputed non-integer CCI values are retained as-is.
# ============================================================
for _col in ['Age', 'CCI']:
    if _col not in data.columns:
        raise KeyError(f'缺少必要列: {_col}')
    data[_col] = pd.to_numeric(data[_col], errors='coerce')

if data['Age'].isna().any():
    raise ValueError(f"Age存在缺失/非数值: {int(data['Age'].isna().sum())}")
if data['CCI'].isna().any():
    raise ValueError(f"CCI存在缺失/非数值: {int(data['CCI'].isna().sum())}")

def _acci_age_score(age):
    if age < 50:
        return 0
    if age < 60:
        return 1
...
data['CCI_age_score'] = data['Age'].apply(_acci_age_score).astype(int)
data['CCI_unadjusted'] = data['CCI'] - data['CCI_age_score']

if (data['CCI_unadjusted'] < 1).any():
    bad = data.loc[data['CCI_unadjusted'] < 1, ['Age','CCI','CCI_age_score','CCI_unadjusted']].head(20)
    raise ValueError(f'发现CCI_unadjusted < 1，停止分析。示例:\n{bad}')

_recon_err = float(np.max(np.abs(data['CCI'] - (data['CCI_unadjusted'] + data['CCI_age_score']))))
if _recon_err > 1e-10:
    raise ValueError(f'CCI拆分重构失败，最大误差={_recon_err}')

_frac_n = int((np.abs(data['CCI'] - np.round(data['CCI'])) > 1e-8).sum())
print('\n✅ CCI decomposition audit passed')
print(f"   CCI_unadjusted range: {data['CCI_unadjusted'].min():.2f} - {data['CCI_unadjusted'].max():.2f}")
print(f"   Age range: {data['Age'].min():.0f} - {data['Age'].max():.0f}")
print(f"   Non-integer imputed CCI values retained: {_frac_n}")
print(f"   Reconstruction max error: {_recon_err}")
...
FORCE_CONTINUOUS = {'Age', 'CCI_unadjusted'}
for col in FEATURES_USED:
    if col in FORCE_CONTINUOUS:
        continuous_features.append(col)
    elif train_df[col].nunique(dropna=True) <= 5 or train_df[col].dtype == object:
...
save_csv(data, '建模数据集_CCI分解12features_固定原始TrainTest身份.csv')
...
with open(os.path.join(output_path, '00_分析设计_CCI分解12变量敏感性分析.json'), 'w', encoding='utf-8') as f:
    import json
    ...
    if os.path.exists(cpath):
        try:
            ...
            if saved.get('features') != FEATURES_USED or saved.get('test_row_ids') != test_ids.tolist():
                raise ValueError('断点的变量集或Test身份与当前CCI分解12变量敏感性分析不一致')
...
# ============================================================
# Reviewer-specific paired comparison:
# Primary 11-predictor model (age-adjusted CCI) vs
# CCI-decomposed 12-predictor model (unadjusted CCI + Age)
# ============================================================
print("\n" + "="*90)
print("Reviewer sensitivity comparison: Primary11 CCI vs CCI-decomposed12")
print("="*90)

def _normalize_id_for_merge(s):
    x = pd.Series(s).copy()
    num = pd.to_numeric(x, errors='coerce')
    if num.notna().all() and np.all(np.isclose(num.values, np.round(num.values))):
        return pd.Series(np.round(num.values).astype(np.int64).astype(str), index=x.index)
    return x.astype(str).str.strip()

def _find_primary11_prediction_file():
    ...
    for root, dirs, files in os.walk(data_path):
        # Skip the current sensitivity-analysis output directory
        if os.path.abspath(root).startswith(os.path.abspath(output_path)):
            continue
        if '测试集逐患者预测概率_校准后_带StudyRowID.csv' in files:
            candidates.append(os.path.join(root, '测试集逐患者预测概率_校准后_带StudyRowID.csv'))

    if not candidates:
        return None

    # Prefer a path containing the primary-analysis folder name.
    candidates.sort(key=lambda p: ('主分析11变量' in p, os.path.getmtime(p)), reverse=True)
    return candidates[0]

def _paired_bootstrap_differences(y_true, p_primary, p_decomp, n_boot=1000, seed=42):
    y_true = np.asarray(y_true, dtype=int)
    p_primary = np.asarray(p_primary, dtype=float)
    p_decomp = np.asarray(p_decomp, dtype=float)
    n = len(y_true)
    rng = np.random.RandomState(seed)
    vals_auc, vals_ap, vals_brier = [], [], []
    ...
    for _ in range(n_boot):
        ...
        p11 = p_primary[idx]
        p12 = p_decomp[idx]
        vals_auc.append(roc_auc_score(yy, p12) - roc_auc_score(yy, p11))
        vals_ap.append(average_precision_score(yy, p12) - average_precision_score(yy, p11))
        vals_brier.append(brier_score_loss(yy, p12) - brier_score_loss(yy, p11))
    ...
    return {
        'Delta_AUC_CI': _ci(vals_auc),
        'Delta_AP_CI': _ci(vals_ap),
        'Delta_Brier_CI': _ci(vals_brier),
        'N_successful_bootstrap': len(vals_auc),
    }

primary_pred_path = _find_primary11_prediction_file()
if primary_pred_path is None:
    print("⚠ 未找到原主分析逐患者预测文件。")
    print("  新的12变量模型已经完整运行并保存；")
    print("  若要自动生成Primary11 vs Decomposed12配对比较，请把原主分析文件")
    print("  测试集逐患者预测概率_校准后_带StudyRowID.csv 放回主分析输出目录后重新运行本脚本。")
else:
    print(f"✅ 找到Primary11预测文件: {primary_pred_path}")
    p11_df = pd.read_csv(primary_pred_path, encoding='utf-8-sig')
    p11_df.columns = p11_df.columns.str.replace('\ufeff','', regex=False).str.strip()

    current_df = pd.DataFrame({
        ID_COL: test_ids.values,
        'True Label Current': y_test.values,
    })
    for _m in model_order:
        current_df[f'{MODEL_FULL_NAME[_m]} Prob Decomposed12'] = test_probas[_m]

    if ID_COL not in p11_df.columns:
        raise KeyError(f'Primary11预测文件缺少 {ID_COL}')
    if 'True Label' not in p11_df.columns:
        raise KeyError('Primary11预测文件缺少 True Label')

    p11_df = p11_df.copy()
    p11_df['_merge_id'] = _normalize_id_for_merge(p11_df[ID_COL])
    current_df['_merge_id'] = _normalize_id_for_merge(current_df[ID_COL])

    if p11_df['_merge_id'].duplicated().any():
        raise ValueError('Primary11预测文件存在重复Study_row_id')
    if current_df['_merge_id'].duplicated().any():
        raise ValueError('当前Decomposed12 Test存在重复Study_row_id')

    keep_primary_cols = ['_merge_id', 'True Label']
    for _m in model_order:
        _col = f'{MODEL_FULL_NAME[_m]} Prob'
        if _col not in p11_df.columns:
            raise KeyError(f'Primary11预测文件缺少概率列: {_col}')
        keep_primary_cols.append(_col)

    merged = current_df.merge(
        p11_df[keep_primary_cols],
        on='_merge_id',
        how='inner',
        validate='one_to_one'
    )
    ...
    if len(merged) != 1011:
        raise ValueError(f'Primary11与Decomposed12合并后应为1011例，实际={len(merged)}')
    if int(merged['True Label Current'].sum()) != 407:
        raise ValueError('合并后事件数不是407')
    if not np.array_equal(
        merged['True Label Current'].astype(int).values,
        merged['True Label'].astype(int).values
    ):
        raise ValueError('Primary11与Decomposed12的True Label不一致')

    y_cmp = merged['True Label Current'].astype(int).values
    comparison_rows = []
    raw_pvals = []

    for _m in model_order:
        label = MODEL_FULL_NAME[_m]
        p11 = merged[f'{label} Prob'].astype(float).values
        p12 = merged[f'{label} Prob Decomposed12'].astype(float).values

        auc11 = roc_auc_score(y_cmp, p11)
        auc12 = roc_auc_score(y_cmp, p12)
        ap11 = average_precision_score(y_cmp, p11)
        ap12 = average_precision_score(y_cmp, p12)
        br11 = brier_score_loss(y_cmp, p11)
        br12 = brier_score_loss(y_cmp, p12)

        cal11 = calibration_intercept_slope(y_cmp, p11)
        cal12 = calibration_intercept_slope(y_cmp, p12)

        # p12 first, p11 second: reported delta is Decomposed12 - Primary11
        _a12, _a11, z, p = delong_test(y_cmp, p12, p11)
        raw_pvals.append(p)

        boot = _paired_bootstrap_differences(
            y_cmp, p11, p12, n_boot=1000, seed=42
        )

        comparison_rows.append({
            'Model': label,
            'Primary11_AUC': auc11,
            'Decomposed12_AUC': auc12,
            'Delta_AUC_12_minus_11': auc12 - auc11,
            'Delta_AUC_boot95_low': boot['Delta_AUC_CI'][0],
            'Delta_AUC_boot95_high': boot['Delta_AUC_CI'][1],
            'DeLong_Z': z,
            'DeLong_P_raw': p,
            'Primary11_AP': ap11,
            'Decomposed12_AP': ap12,
            'Delta_AP_12_minus_11': ap12 - ap11,
            'Delta_AP_boot95_low': boot['Delta_AP_CI'][0],
            'Delta_AP_boot95_high': boot['Delta_AP_CI'][1],
            'Primary11_Brier': br11,
            'Decomposed12_Brier': br12,
            'Delta_Brier_12_minus_11': br12 - br11,
            'Delta_Brier_boot95_low': boot['Delta_Brier_CI'][0],
            'Delta_Brier_boot95_high': boot['Delta_Brier_CI'][1],
            'Primary11_Calibration_intercept': cal11['intercept'],
            'Primary11_Calibration_slope': cal11['slope'],
            'Decomposed12_Calibration_intercept': cal12['intercept'],
            'Decomposed12_Calibration_slope': cal12['slope'],
            'Bootstrap_successful_n': boot['N_successful_bootstrap'],
        })

    cmp_df = pd.DataFrame(comparison_rows)
    cmp_df['DeLong_P_Holm'] = holm_adjust_pvalues(np.asarray(raw_pvals, dtype=float))
    cmp_df['Holm_significant_0.05'] = cmp_df['DeLong_P_Holm'] < 0.05

    # Put LR near the top for reviewer-focused reading without altering computations.
    _sort_order = {MODEL_FULL_NAME[m]: i for i, m in enumerate(model_order)}
    cmp_df['_display_order'] = cmp_df['Model'].map(
        lambda x: -1 if x == 'Logistic Regression' else _sort_order.get(x, 999)
    )
    cmp_df = cmp_df.sort_values('_display_order').drop(columns='_display_order').reset_index(drop=True)

    cmp_path = os.path.join(
        output_path,
        'Reviewer_CCI分解_Primary11_vs_Decomposed12_配对比较.csv'
    )
    cmp_df.to_csv(cmp_path, index=False, encoding='utf-8-sig')

    patient_path = os.path.join(
        output_path,
        'Reviewer_CCI分解_逐患者Primary11_vs_Decomposed12.csv'
    )
    merged.to_csv(patient_path, index=False, encoding='utf-8-sig')

    print("\n✅ Reviewer-specific paired comparison saved")
    print(f"   {cmp_path}")
    print(f"   {patient_path}")
    print("\n核心结果：")
    show_cols = [
        'Model', 'Primary11_AUC', 'Decomposed12_AUC',
        'Delta_AUC_12_minus_11', 'DeLong_P_raw', 'DeLong_P_Holm',
        'Primary11_Brier', 'Decomposed12_Brier',
        'Primary11_Calibration_intercept', 'Primary11_Calibration_slope',
        'Decomposed12_Calibration_intercept', 'Decomposed12_Calibration_slope'
    ]
    print(cmp_df[show_cols].to_string(index=False))

print("\n" + "="*90)
print("CCI decomposition sensitivity analysis complete.")
print("="*90)
